...
router.post('/upload', async (req,res)=>{
  ...
  if(file.size >= (1024*1024*200)){
    return res.status(400).json({message: 'file too large, it must be less than 200mb'})
  }
  try {
    let fileName = await Files.findOne({name: file.name})
    if(fileName){
      return res.status(400).json({message: 'file name already existe'})
    }
    ...
    await newFile.save()
    res.json({name: newFile.name, url: `${req.protocol}://${req.get('host')}/${newFile.url}`})
